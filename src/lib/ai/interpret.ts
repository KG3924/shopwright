  ...
  .handler(async ({ data }) => {
    try {
      ...
      const prompt = [
        ...
        "drawing.backStyle (lattice|splat|solid|crest|none|unknown), seat (solid|upholstered|unknown), and finish (paint|clear|unknown) are required. Unknown axes are value null, source unknown. Do not invent lattice, enamel, primer, stock 3/4 thickness, or a joinery route.",
      ]
        .filter(Boolean)
        .join("\n");
