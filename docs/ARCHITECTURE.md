...
If `templateId` matches a studio piece it is **ignored on photo / URL / blueprint hydrate**. Joinery, hardware, steps, finish, and technique plates compile from this interpret only (parts, `drawing.backStyle === "lattice"` for lattice plates, `seat` / `finish`, uncertainties). Lattice hardware/steps/figures attach only when interpret tagged `drawing.backStyle` as lattice — splat, solid, crest, none, and unknown do not. Missing backStyle is not lattice. Upholstered seat compiles a plywood / webbing / foam / fabric pack, not a glue-up to paint. Unknown finish stays unknown.
